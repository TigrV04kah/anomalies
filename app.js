...
Object.assign(CHECK_HELP, {
  ...
  bounded_score_total_consistency: {
    ...
    short: "Checks Volleyball Total_B through a bounded score grid.",
    long: "For Volleyball periods 1, 2, 3, 4, 5, the check builds valid score grids, normalizes B/M probabilities, fits the grid to individual totals, and compares model P(Total_B) with market P(Total_B). Period 0 is excluded. Threshold: Volleyball 18.5 p.p. Tennis is disabled for this rule and will need separate research."
  },
  stat_conflicts: {
  ...
  individual_total_favorite_consistency: {
    ...
    long: "Для каждого GameID, где есть p1/p2 и IndTotal_1_B/IndTotal_2_B, определяется фаворит ниже 1.8. При одинаковом Param коэффициент фаворита должен быть ниже коэффициента аутсайдера. Если дельта вероятностей индивидуальных тоталов не выше 1.5 п.п. или один из коэффициентов ниже 1.1, сигнал сохраняется как SOFT и выводится только во вкладке Soft. Исключение: если разница implied probability между фаворитом и аутсайдером матча не меньше 15 п.п., сигнал остается hard DIFF. Если одинакового Param нет, сравниваются центральные линии сторон: у каждой стороны берется Param с коэффициентом, ближайшим к вероятности 0.5. Центральный Param фаворита должен быть выше центрального Param аутсайдера. Для центрального сценария дельта Param до 0.5 при разнице вероятностей до 20 п.п. тоже считается SOFT, кроме случаев с сильным матчевым фаворитом 15+ п.п."
  },
